  (function () {
    ...
    document.head.insertAdjacentHTML("beforeend", "<style>[data-hide-if] {display: none;}</style>");
    onDomReady(() => {
      let visibilityStyles = Array.from(document.querySelectorAll("[data-hide-if]")).map(showIfElem => {
        let hideSelector = showIfElem.getAttribute("data-hide-if");
        let hideStyles = "";

        if (hideSelector) {
          hideStyles = `:not(${hideSelector}) [data-hide-if="${hideSelector}"], [data-hide-if="${hideSelector}"]:not(${hideSelector}) {display: block;}`;
        }
